...
First, we will simulate some spatial data with three cell types: (A), (B), and (C). The first group of samples will be randomly dispersed in the tissue area, while the second group of samples with exhibit more clustering of cell types. We will also create a design matrix to keep track of which samples belong to each group. 
```{r}
...
```
Next, we will run the PANORAMIC workflow. We will specify the spatial distance of colocalization a 10 micron, and we will use the Ripley's cross-L function as our spatial statistic. For now, we will use 50 simulations for our spatial bootstrapping, but when running this on real data, we recommend running nim >= 100. 
```{r}
# Preparation step and calculation of spatial statistics + bootstrapping. 
se <- panoramic(
  spe_list,
  design      = design,
  cell_type   = "cell_type",
...
# Meta-analysis step to pool estimates robustly across multiple spatial samples. 
se_meta <- panoramic_meta(se, tau2 = "SJ", group_col = "group")

# Differential testing step to compare colocalization estimates between groups. 
...
We can also visualize our differential testing results as a network and perform clustering to discover subnetworks of colocalization across mutliple cell types. In this toy example we only have three cell types, but in larger settings, you can see colocalization subnetworks of 2+ cell types. 
```{r}
